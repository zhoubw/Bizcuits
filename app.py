    ...
    if request.method == "POST":
        ...
        if response != []:
            ...
            if 'username' in session:
                rates = users.find_one({'username':session['username']})['rates']
                rates_str = [str(x) for x in rates]
                return render_template("results.html", locations=sortedlocs, session=session, users=users, get_timestamp=get_timestamp, get_votes=database.get_votes, rates=rates_str, isError=isError)
